...
import org.apache.shindig.gadgets.oauth2.AccessorInfo;
...
import org.apache.shindig.gadgets.oauth2.OAuth2;
import org.apache.shindig.gadgets.oauth2.OAuth2ClientState;
...
public class BasicOAuth2Request implements
		org.apache.shindig.gadgets.oauth2.OAuth2Request {
	 ...
	 */
	protected OAuth2ClientState clientState;
	 ...
	 */
	public BasicOAuth2Request(OAuth2FetcherConfig fetcherConfig,
			HttpFetcher fetcher) {
	 ...
	 *            fetcher to use for actually making requests
	 * @param trustedParams
	 *            additional parameters to include in all outgoing OAuth
	 *            requests, useful for client data that can't be pulled from the
	 *            security token but is still trustworthy.
	 */
	public BasicOAuth2Request(OAuth2FetcherConfig fetcherConfig,
	...
	public HttpResponse fetch(HttpRequest request) {
		realRequest = request;
		clientState = new OAuth2ClientState(fetcherConfig.getStateCrypter(),
				request.getOAuth2Arguments().getOrigClientState());
		responseParams = new OAuth2ResponseParams(request.getSecurityToken(),
		...
		try {
			...
			// failure.
			responseParams.logDetailedWarning(classname, "fetch",
	...
	private HttpResponse fetchNoThrow() {
		...
		try {
			accessorInfo = fetcherConfig.getTokenStore().getOAuthAccessor(
					...
					realRequest.getOAuth2Arguments(), clientState,
			...
			if (OAuth2Error.UNAUTHENTICATED.name().equals(e.getError())) {
				responseParams.logDetailedInfo(classname, "fetchNoThrow",
						MessageKeys.UNAUTHENTICATED_OAUTH, e);
			} else if (OAuth2Error.BAD_OAUTH_TOKEN_URL.name().equals(
					e.getError())) {
				responseParams.logDetailedInfo(classname, "fetchNoThrow",
						MessageKeys.INVALID_OAUTH, e);
			} else {
				responseParams.logDetailedWarning(classname, "fetchNoThrow",
	...
	private boolean handleProtocolException(OAuthProtocolException pe,
			int attempts) throws OAuth2RequestException {
		if (pe.canExtend()) {
			...
			// accessorInfo.getConsumer(), realRequest.getOAuth2Arguments(),
			// responseParams);
	...
	/**
	 * Does one of the following: 1) Sends an access token request for an access
	 * token 2) Asks for data from the service provider.
	 */
	 ...
	 */
	private void checkCanApprove() throws OAuth2RequestException {
	...
	/*
	 * private void fetchRequestToken() throws OAuth2RequestException,
	 * OAuthProtocolException { OAuthAccessor accessor =
	 * accessorInfo.getAccessor(); HttpRequest request =
	 * createRequestTokenRequest(accessor);
	 * 
	...
	private void addCallback(List<Parameter> requestTokenParams)
		...
		// callback URL.
		String baseCallback = StringUtils.trimToNull(accessorInfo.getConsumer()
				.getCallbackUrl());
		if (baseCallback != null) {
			String callbackUrl = fetcherConfig.getOAuthCallbackGenerator()
					.generateCallback(fetcherConfig, baseCallback, realRequest,
							responseParams);
			if (callbackUrl != null) {
	...
	private List<Parameter> sanitize(List<Parameter> params)
		...
		for (Parameter p : params) {
			...
			if (allowParam(name)) {
				...
				throw new OAuth2RequestException(OAuth2Error.INVALID_PARAMETER,
						name);
			}
		}
		return list;
	...
	protected boolean allowParam(String paramName) {
		...
		return (!(canonParamName.startsWith("oauth")
				|| canonParamName.startsWith("xoauth") || canonParamName
				.startsWith("opensocial")) && ALLOWED_PARAM_NAME.matcher(
	...
	private void overrideParameters(List<Parameter> authParams)
		...
		for (Parameter param : trustedParams) {
			if (!isContainerInjectedParameter(param.getKey())) {
				throw new OAuth2RequestException(
						OAuth2Error.INVALID_TRUSTED_PARAMETER, param.getKey());
			}
			paramMap.put(param.getKey(), param.getValue());
		}
	...
	private void addSignatureParams(List<Parameter> params) {
		if (accessorInfo.getConsumer().getConsumer().consumerKey == null) {
			params.add(new Parameter(OAuth.OAUTH_CONSUMER_KEY, realRequest
					.getSecurityToken().getDomain()));
		}
		if (accessorInfo.getConsumer().getKeyName() != null) {
			params.add(new Parameter(XOAUTH_PUBLIC_KEY_OLD, accessorInfo
					.getConsumer().getKeyName()));
			params.add(new Parameter(XOAUTH_PUBLIC_KEY_NEW, accessorInfo
					.getConsumer().getKeyName()));
		}
		params.add(new Parameter(OAuth.OAUTH_VERSION, OAuth.VERSION_1_0));
		params.add(new Parameter(OAuth.OAUTH_TIMESTAMP, Long
				.toString(fetcherConfig.getClock().currentTimeMillis() / 1000L)));
		// the oauth.net java code uses a clock to generate nonces, which causes
		// nonce collisions
		// under heavy load. A random nonce is more reliable.
		params.add(new Parameter(OAuth.OAUTH_NONCE, String.valueOf(Math
				.abs(Crypto.RAND.nextLong()))));
	}
	...
	public HttpRequest sanitizeAndSign(HttpRequest base,
		...
		case URL_AND_FORM_PARAMS:
			try {
				params.addAll(sanitize(OAuth.decodeForm(base
						.getPostBodyAsString())));
			} catch (IllegalArgumentException e) {
				// Occurs if OAuth.decodeForm finds an invalid URL to decode.
				throw new OAuth2RequestException(OAuth2Error.INVALID_REQUEST,
		...
		case URL_AND_BODY_HASH:
			try {
				...
				String b64 = CharsetUtil.newUtf8String(Base64
						.encodeBase64(hash));
		...
		try {
			OAuthMessage signed = newRequestMessage(
	...
	private HttpRequest createHttpRequest(HttpRequest base,
		...
		OAuth2ParamLocation paramLocation = accessorInfo.getParamLocation();
		...
		if (paramLocation == OAuth2ParamLocation.POST_BODY
				&& !result.getMethod().equals("POST")) {
			paramLocation = OAuth2ParamLocation.AUTH_HEADER;
		}
		...
		case URI_QUERY:
			result.setUri(Uri.parse(OAuthUtil.addParameters(result.getUri()
					.toString(), oauthParams)));
			break;
		}
	...
	/**
	 * Sends OAuth access token messages.
	 */
	private OAuthMessage sendOAuthMessage(HttpRequest request)
			throws OAuth2RequestException, OAuthProtocolException {
		...
		if (OAuthUtil.getParameter(reply, OAuth.OAUTH_TOKEN) == null) {
			throw new OAuth2RequestException(
					OAuth2Error.MISSING_OAUTH_PARAMETER, OAuth.OAUTH_TOKEN);
		}
		if (OAuthUtil.getParameter(reply, OAuth.OAUTH_TOKEN_SECRET) == null) {
			throw new OAuth2RequestException(
					OAuth2Error.MISSING_OAUTH_PARAMETER,
					OAuth.OAUTH_TOKEN_SECRET);
		}
		return reply;
	}
	...
	/**
	 * Builds the URL the client needs to obtain both access tokens and refresh
	 * tokens This is section 4.1.1 of OAuth2.0 spec
	 */
	private void buildAznUrl() throws OAuth2RequestException {
		// We add the token, gadget is responsible for the callback URL.
		OAuth2Accessor accessor = accessorInfo.getAccessor();
		if (accessor.consumer.serviceProvider.authorizationURL == null) {
			throw new OAuth2RequestException(OAuth2Error.BAD_OAUTH_TOKEN_URL,
					"authorization");
		}
		StringBuilder azn = new StringBuilder(
				accessor.consumer.serviceProvider.authorizationURL);
		if (azn.indexOf("?") == -1) {
			azn.append('?');
		} else {
			azn.append('&');
		...
		// append client_id,response_type,callbackurl, state
		azn.append("");
		azn.append(OAuth2.PARAM_RESPONSE_TYPE);
		azn.append('=');
		azn.append(OAuth2.ResponseType.code.name());

		azn.append("&");
		azn.append(OAuth2.PARAM_CLIENT_ID);
		azn.append('=');
		azn.append(OAuth.percentEncode(accessor.consumer.consumerKey));

		if (accessor.consumer.callbackURL != null) {
			azn.append("&");
			azn.append(OAuth2.PARAM_REDIRECT_URI);
			azn.append('=');
			azn.append(OAuth.percentEncode(accessor.consumer.callbackURL));
		}
		// todo !!! need to add scope/state support
...
//				&& accessorInfo.getAccessor().requestToken != null
	...
	private boolean accessTokenExpired() {
		return (accessorInfo.getTokenExpireMillis() != ACCESS_TOKEN_EXPIRE_UNKNOWN && accessorInfo
				.getTokenExpireMillis() < fetcherConfig.getClock()
				.currentTimeMillis());
	}

	/**
	 * Implements section 4.4.2 of the OAuth2.0 spec.
	 */
	private void getAccessToken() throws OAuth2RequestException,
			OAuthProtocolException {
		...
		Uri accessTokenUri = Uri
				.parse(accessor.consumer.serviceProvider.accessTokenURL);
		...
		msgParams.add(new Parameter(OAuth2.PARAM_CODE,
		...
		String receivedCallback = realRequest.getOAuthArguments()
				.getReceivedCallbackUrl();
		if (!StringUtils.isBlank(receivedCallback)) {
			try {
				Uri parsed = Uri.parse(receivedCallback);
				String verifier = parsed
						.getQueryParameter(OAuth.OAUTH_VERIFIER);
				if (verifier != null) {
					msgParams
							.add(new Parameter(OAuth.OAUTH_VERIFIER, verifier));
				}
			} catch (IllegalArgumentException e) {
				throw new OAuth2RequestException(OAuth2Error.INVALID_REQUEST,
		...
		if (OAuthUtil.getParameter(reply, OAuthConstants.OAUTH_EXPIRES_IN) != null) {
			try {
				...
				long expireMillis = fetcherConfig.getClock()
						.currentTimeMillis() + expireSecs * 1000L;
				...
				responseParams.logDetailedWarning(classname,
						"exchangeRequestToken", MessageKeys.BOGUS_EXPIRED);
			}
		}
	 ...
	 */
	private void saveAccessToken() throws OAuth2RequestException {
		OAuth2Accessor accessor = accessorInfo.getAccessor();
	 ...
	 *             if the service provider returns an OAuth related error
	...
	private HttpResponse fetchFromServer(HttpRequest request)
		...
		try {
			...
			throw new OAuth2RequestException(
					OAuth2Error.MISSING_SERVER_RESPONSE, "", e);
		} finally {
	...
	private void checkForProtocolProblem(HttpResponse response)
			throws OAuthProtocolException {
		if (couldBeFullOAuth2Error(response)) {
	 ...
	 */
	private boolean couldBeFullOAuth2Error(HttpResponse response) {
	 ...
	 * 
	 * @param message
	 *            the OAuthMessage object, which holds non-OAuth parameters such
	 *            as foo=bar (which may have been in the original URI query
	...
	public static OAuthMessage newRequestMessage(OAuth2Accessor accessor,
